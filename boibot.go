...
func reportCovidTH(replyToken string, text []string) bool {
	...
	if code == "TH" {
		...
		message := fmt.Sprintf("ยืนยันผู้ติดเชื้อ Covid19  \U0001f9a0\n" +
			...
			"%s",
			country.CountryTh, country.Emoji ,reportData[len(reportData)-1].UpdateDate[0:10], reportData[len(reportData)-1].Confirmed,
			reportData[len(reportData)-1].NewConfirmed, PercentCaseIncrease, "%", reportData[len(reportData)-1].Hospitalized,
			reportData[len(reportData)-1].Recovered, reportData[len(reportData)-1].Deaths, reportData[len(reportData)-1].NewDeaths,
...
func reportCovidEN(replyToken string, text []string) bool {
	...
	if code == "TH" {
		...
		message := fmt.Sprintf("Covid19  \U0001f9a0  in Thailand  %s  \n" +
			...
			"%s",
			country.Emoji ,reportData[len(reportData)-1].UpdateDate[0:10], reportData[len(reportData)-1].Confirmed,
			reportData[len(reportData)-1].NewConfirmed, PercentCaseIncrease, "%", reportData[len(reportData)-1].Hospitalized,
			reportData[len(reportData)-1].Recovered, reportData[len(reportData)-1].Deaths, reportData[len(reportData)-1].NewDeaths,
			PercentDeathIncrease, "%", Credit)
		sendReplyMessage(replyToken, message)
